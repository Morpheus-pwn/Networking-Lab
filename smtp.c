...
char * Mail_Header(char *from,char *to,char *sub,char *content)
{
 ...
 sprintf(content_a,"%s\r\n",content);
 int header_length = strlen(DATE_h) + strlen(sender) + strlen(subject) +
strlen(recep) + strlen(content_a);
 ...
 memcpy(&header[0 + strlen(DATE_h) + strlen(sender)], &subject ,
strlen(subject));
 memcpy(&header[0 + strlen(DATE_h) + strlen(sender) + strlen(subject)], &recep ,
strlen(recep));
 memcpy(&header[0 + strlen(DATE_h) + strlen(sender) + strlen(subject) +
strlen(recep)], &content_a , strlen(content_a));
return header;
}
...
>h_length);
 ...
 int connect_id;
 connect_id = connect(socket_id,(struct sockaddr *)&serv_addr,
sizeof(serv_addr));
 if(connect_id < 0)
 {
...
strcpy(buff, Mail_Header(mail_from,mail_to,sub,content)); //assigning
header to buffer
